...
export interface Bill {
    bill_id: number;
    bill_type: 'consultation' | 'medication';
    total_amount: number;
    tax_amount: number;
...
function Bill() {
    ...
    const [billTypeTab, setBillTypeTab] = useState<number>(0); // 0: All, 1: Consultation, 2: Medication
    ...
    const getCurrentBills = () => {
        switch (billTypeTab) {
            case 1:
                return billsData.consultationBills;
            case 2:
                return billsData.medicationBills;
            default:
                return billsData.allBills;
        }
    };
    ...
    const handlePaymentSuccess = (billId: number,appointmentInfo:any,billType:string) => {
        setBillsData((prev) => ({
            ...
            consultationBills: prev.consultationBills.map((bill) =>
                bill.bill_id === billId ? { ...bill, payment_status: "paid" } : bill
            ),
            medicationBills: prev.medicationBills.map((bill) =>
                bill.bill_id === billId ? { ...bill, payment_status: "paid" } : bill
            )
        }));
    ...
    const handleDownload = () => {
        ...
        doc.text(`Bill Type: ${bill.bill_type.toUpperCase()}`, 14, 59);
        doc.text(`Billing Date: ${new Date(bill.billing_date).toLocaleDateString()}`, 14, 66);
        doc.text(`Payment Status: ${bill.payment_status.toUpperCase()}`, 14, 73);
        ...
        // 🔹 TOTALS SECTION
        let finalY = (doc as any).lastAutoTable.finalY || 97;
        doc.setFontSize(12);
        doc.setFont("helvetica", "normal");
        ...
        // Save PDF
        doc.save(`Receipt_${bill.bill_type}_${bill.bill_id}.pdf`);
    };
        ...
        <Box sx={{ width: "100%", p: 2 }}>
            ...
            <Grow in timeout={600}>
                ...
                >
                    <Tabs
                        value={billTypeTab}
                        onChange={(_, newValue) => setBillTypeTab(newValue)}
                        variant="fullWidth"
                        sx={{
                            '& .MuiTab-root': {
                                textTransform: 'none',
                                    ...
                                    <Grid size={{ xs: 12, md: 6 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
                                            ...
                                            <Box>
                                                ...
                                                <Typography variant="body2" color="text.secondary">
                                                    Bill #{bill.bill_id} • {bill.bill_type}
                                                </Typography>
                                            </Box>
                                        </Box>
                                    </Grid>
            ...
            <CustomModal
                open={open}
                onClose={handleClose}
                title={`Bill #${selectedBill?.bill_id} - ${selectedBill?.bill_type}`}
                content={
                    selectedBill && (
                        <Box sx={{ p: { xs: 1, sm: 2 } }}>
                            ...
                            >
                                ...
                                <Grid container spacing={2} sx={{ mt: 0.5 }}>
                                    ...
                                    <Grid size={{ xs: 6 }}>
                                        ...
                                        <Chip
                                            label={selectedBill.bill_type}
                                            size="small"
                                            sx={{
                                                fontWeight: '600',
                                                backgroundColor: selectedBill.bill_type === 'consultation' ?
                                                    'rgba(25, 118, 210, 0.1)' : 'rgba(156, 39, 176, 0.1)',
                                                color: selectedBill.bill_type === 'consultation' ?
                                                    '#1976d2' : '#9c27b0'
                                            }}
                                        />
                                    </Grid>
